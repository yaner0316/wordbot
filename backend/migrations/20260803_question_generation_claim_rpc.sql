...
drop function if exists public.claim_question_generation_jobs(
...
begin
    ...
    if not found
       ...
       or not (v_job.lease_expires_at > v_now) then
        return;
    end if;
...
revoke all on function public.renew_question_generation_job(uuid, text, bigint)
    from public, anon, authenticated;
grant execute on function public.renew_question_generation_job(uuid, text, bigint)
    to service_role;

revoke all on function public.publish_question_generation_variants(uuid, text, jsonb)
    from public, anon, authenticated;
grant execute on function public.publish_question_generation_variants(uuid, text, jsonb)
    to service_role;

revoke all on function public.complete_question_generation_job(uuid, text)
    from public, anon, authenticated;
grant execute on function public.complete_question_generation_job(uuid, text)
    to service_role;

revoke all on function public.fail_question_generation_job(uuid, text, integer, bigint, bigint, text, text, jsonb)
    from public, anon, authenticated;
grant execute on function public.fail_question_generation_job(uuid, text, integer, bigint, bigint, text, text, jsonb)
    to service_role;
