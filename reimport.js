...
async function main() {
    ...
    existingWords.forEach(r => {
        const w = r.fields.Word || '';
        const u = r.fields.user || '';
        existingMap.set(w.toLowerCase() + '_' + u, true);
    });
    
    const allUsers = new Set(['yusi', 'qiuqiu']);
    const userStats = { yusi: { total: 0, existing: 0, added: 0 }, qiuqiu: { total: 0, existing: 0, added: 0 } };
    ...
    for (const row of rows) {
        ...
        let importer = row['录入人'] || '';
        const user = (importer === 'qiuqiu' ? 'qiuqiu' : 'yusi');
        allUsers.add(user);
        userStats[user].total++;
        
        if (existingMap.has(word.toLowerCase() + '_' + user)) {
            userStats[user].existing++;
            continue;
        }
        ...
        const pos = word.endsWith('tion') || word.endsWith('ment') ? 'n' : word.endsWith('ly') ? 'adv' : 'adj';
        ...
        existingMap.set(word.toLowerCase() + '_' + user, true);
        userStats[user].added++;
    }
    
    console.log('\n========== 导入结果 ==========');
    for (const [user, stats] of Object.entries(userStats)) {
        console.log(`${user}: 总计${stats.total} | 已存在${stats.existing} | 新增${stats.added}`);
    }
}
