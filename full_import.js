...
async function deleteRecord(appToken, tableId, recordId) {
    const token = await getToken();
    return request('DELETE', '/open-apis/bitable/v1/apps/' + appToken + '/tables/' + tableId + '/records/' + recordId, null, token);
}
...
async function main() {
    console.log('1. 读取Excel...');
    const workbook = XLSX.readFile('单词机器人-数据 副本_生产-单词表.xlsx');
    const rows = XLSX.utils.sheet_to_json(workbook.Sheets[workbook.SheetNames[0]]);
    console.log('   Excel行数:', rows.length);
    
    console.log('2. 清空旧数据...');
    let wordRecords = await getRecords(WORD_APP, WORD_TABLE);
    console.log('   单词表记录:', wordRecords.length);
    for (const r of wordRecords) {
        await deleteRecord(WORD_APP, WORD_TABLE, r.record_id);
    }
    
    let distRecords = await getRecords(DIST_APP, DIST_TABLE);
    console.log('   干扰项表记录:', distRecords.length);
    for (const r of distRecords) {
        await deleteRecord(DIST_APP, DIST_TABLE, r.record_id);
    }
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (!userStats[user]) userStats[user] = { total: 0, mastered: 0 };
        ...
        await addRecord(WORD_APP, WORD_TABLE, {
            'user': user,
            'Word': word,
        ...
        await addRecord(DIST_APP, DIST_TABLE, {
            'Word': word,
            'POS': pos,
        ...
        if ((i + 1) % 50 === 0) console.log('   已导入', i + 1, '条...');
    }
    
    console.log('\n4. 导入完成:');
    for (const [user, stats] of Object.entries(userStats)) {
        console.log('   ' + user + ': 总' + stats.total + ' 已掌握' + stats.mastered);
    }
}
